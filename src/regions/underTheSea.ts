...
import type { Region, Theme } from '@/types.d'

import aquaSvgUrl from '@/assets/images/backgrounds/aqua.svg?url'

const { DOT, ANCHOR } = config.tileTypes
const { BLUE, RED, YELLOW, GREEN, PINK } = config.dotColors

const theme: Theme = {
  background: `url(${aquaSvgUrl}) repeat center`
}

export const underTheSeaRegion: Region = {
  name: 'Under the Sea',
